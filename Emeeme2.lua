...
local function hookKnifeThrown()
    ...
    local knifeTool = character:FindFirstChild("Knife") or (Backpack and Backpack:FindFirstChild("Knife"))
    ...
    local isRemoteEvent = knifeThrownEvent:IsA("RemoteEvent")
    local isRemoteFunction = knifeThrownEvent:IsA("RemoteFunction")
    
    if not isRemoteEvent and not isRemoteFunction then return end

    if not originalKnifeThrown then
        if isRemoteEvent then
            originalKnifeThrown = knifeThrownEvent.FireServer
        else
            originalKnifeThrown = knifeThrownEvent.InvokeServer
        end
    end

    local metodoOriginal = isRemoteEvent and "FireServer" or "InvokeServer"
    
    knifeThrownEvent[metodoOriginal] = function(self, ...)
        local args = {...}
...
local function FlingPlayer(targetPlayer)
    ...
    local function flingBasePart(basePart)
        ...
        until basePart.Velocity.Magnitude > -5000 or tick() - start > FLING_TIMEOUT
    end
